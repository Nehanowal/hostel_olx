...
export function guestRoutes({app,db,university,imageStorage}){
 ...
 app.get('/api/guest/listings',async(req,res)=>{
  const {page,q,category}=z.object({page:z.coerce.number().int().min(1).max(10000).default(1),q:z.string().trim().max(100).default(''),category:z.enum(categories.map(c=>c.id)).optional()}).parse(req.query);
  ...
  const rows=await db.prepare(`SELECT l.id,l.title,l.category,l.condition,l.price,l.created_at ${base} ORDER BY CASE WHEN l.category='Fashion' THEN 1 ELSE 0 END,l.created_at DESC,l.id DESC LIMIT 24 OFFSET ?`).all(...args,(page-1)*24);
