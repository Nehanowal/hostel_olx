...
3. Generate a Brevo API key. Add these variables in the existing Render service's **Environment** page (never in GitHub or frontend environment variables):
...
4. Save and redeploy Render. Existing Google, Turso, Cloudinary and proxy variables remain unchanged. No Vercel secrets or additional service are needed.
...
To turn delivery off without affecting chat, set `MESSAGE_EMAIL_ENABLED=false` and redeploy Render.
...
- Render's free instance can sleep after inactivity. Jobs remain durable but processing resumes only when the service wakes. There is no separate always-on worker or paid background service.
